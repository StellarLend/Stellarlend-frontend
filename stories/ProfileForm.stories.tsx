...
export const ErrorStates: Story = {
  play: async ({ canvasElement }) => {
    ...
    const countryError = canvas.getByText(
      /Identification country is required/i,
    );
    ...
    const genderError = canvas.getAllByText(/Gender is required/i);
    await expect(genderError).toHaveLength(2);
  },
};
...
export const PartialFill: Story = {
  play: async ({ canvasElement }) => {
    ...
    await expect(
      canvas.queryByText(/First name is required/i),
...
export const InvalidFormats: Story = {
  play: async ({ canvasElement }) => {
    ...
    const emailFormatError = canvas.getByText(
      /Please enter a valid email address/i,
    );
    await expect(emailFormatError).toBeInTheDocument();

    const phoneFormatError = canvas.getByText(
...
export const FullFill: Story = {
  play: async ({ canvasElement }) => {
    ...
    await userEvent.type(
      canvas.getByLabelText(/Tax Verification Number/i),
      "12-3456789",
    );
    await userEvent.type(
      canvas.getByLabelText(/Identification Country/i),
      "United States",
    );
    await userEvent.type(
    ...
    await expect(
      canvas.queryByText(/Please enter a valid/i),
    ).not.toBeInTheDocument();
  },
};
...
export const ClearErrors: Story = {
  play: async ({ canvasElement }) => {
    ...
    const genderErrors = canvas.getAllByText(/Gender is required/i);
    await expect(genderErrors).toHaveLength(2);
    ...
    const maleRadio = canvas.getByDisplayValue("male");
    await userEvent.click(maleRadio);
    await expect(canvas.queryAllByText(/Gender is required/i)).toHaveLength(0);
  },
};
