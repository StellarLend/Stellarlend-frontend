...
import { Search, ArrowRight, ChevronsUpDown, ListFilter, CalendarDays } from 'lucide-react';
...
const statusOptions: (TransactionStatus | "All")[] = ["All", "Completed", "Processing", "Failed"];
...
export default function RecentTransactions() {
  ...
  useEffect(() => {
    function handleClick(e: MouseEvent) {
      if (showSearch && searchRef.current && !searchRef.current.contains(e.target as Node)) setShowSearch(false);
  ...
  const CustomDateInput = forwardRef<HTMLInputElement, { value: string; onClick: () => void; placeholder: string; icon: React.ReactNode }>(
    ({ value, onClick, placeholder, icon }, ref) => {
      return (
        <div className="relative">
          <span className="absolute left-2 top-1.5 text-gray-400 pointer-events-none mt-[2px]">{ icon }</span>
          <input
            ref={ ref }
            type="text"
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between px-6 pb-2 gap-2">
        <div className="flex gap-6 items-center flex-wrap text-gray-400 font-normal text-base select-none">
          <div className="relative" ref={ searchRef }>
            <div className="flex items-center gap-1 cursor-pointer" onClick={ () => setShowSearch((v) => !v) }>
              <Search size={ 18 } />
              <span>Search</span>
            </div>
            { showSearch && (
          ...
          <div className="relative" ref={ filterRef }>
            <div className="flex items-center gap-1 cursor-pointer" onClick={ () => setShowFilter((v) => !v) }>
              <ListFilter size={ 18 } />
              <span>Filter</span>
            </div>
            { showFilter && (
              <div className="absolute left-0 mt-2 w-32 bg-white border rounded shadow z-10">
                { statusOptions.map((opt) => (
                  <button
                    key={ opt }
                    className={ `block w-full text-left px-4 py-2 text-sm hover:bg-gray-100 ${status === opt ? 'font-bold text-primary-700' : ''}` }
                    onClick={ () => { setStatus(opt); setShowFilter(false); } }
                    type="button"
                  >
                    { opt }
                  </button>
          ...
          <div className="relative" ref={ sortRef }>
            <div className="flex items-center gap-1 cursor-pointer" onClick={ () => setShowSort((v) => !v) }>
            ...
            { showSort && (
              <div className="absolute left-0 mt-2 w-32 bg-white border rounded shadow z-10">
                <button
                  className={ `block w-full text-left px-4 py-2 text-sm hover:bg-gray-100 ${sortBy === 'date' ? 'font-bold text-primary-700' : ''}` }
                  onClick={ () => { setSortBy('date'); setShowSort(false); } }
                  type="button"
                >
                  Date { sortBy === 'date' && (sortDir === 'asc' ? '↑' : '↓') }
                </button>
                <button
                  className={ `block w-full text-left px-4 py-2 text-sm hover:bg-gray-100 ${sortBy === 'amount' ? 'font-bold text-primary-700' : ''}` }
                  onClick={ () => { setSortBy('amount'); setShowSort(false); } }
                  type="button"
                >
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody>
              { filtered.map((txn, idx) => (
                <tr key={ idx } className="border-b border-gray-300 last:border-0 hover:bg-gray-50 transition">
                  <td className="py-3 px-4">
                    <div className="font-medium text-gray-900">{ txn.type }</div>
                    <div className="text-xs text-gray-400">#{ txn.id }</div>
                  </td>
                  <td className="py-3 px-4 font-mono">{ txn.amount > 0 ? `+$${txn.amount}` : `-$${Math.abs(txn.amount)}` }</td>
                  <td className="py-3 px-4 flex items-center gap-2">
                    ...
                    />
                    <span className="ml-1 font-medium text-gray-700">{ txn.asset }</span>
                  </td>
                  <td className="py-3 px-4">
                    { formatDateTime(txn.date, txn.time) }
                  </td>
                  <td className="py-3 px-4">
                    <span className={ `px-2 py-1 rounded text-xs font-semibold ${statusColors[txn.status]}` }>
                      { txn.status }
                    </span>
                  </td>
                </tr>
